...
def recover_table_layout(bounding_boxes):
    ...
    x1_coords = sorted(x1_coords)
    y1_coords = sorted(y1_coords)
    x2_coords = sorted(x2_coords)
    y2_coords = sorted(y2_coords)
    ...
    virtual_vertical_lines1 = group_numbers_by_threshold(x1_coords,x_threshold)
    virtual_horizontal_lines1 = group_numbers_by_threshold(y1_coords,y_threshold)
    virtual_vertical_lines2 = group_numbers_by_threshold(x2_coords,x_threshold)
    virtual_horizontal_lines2 = group_numbers_by_threshold(y2_coords,y_threshold)
    
    virtual_vertical_lines.append(virtual_vertical_lines1[0])
    for i in range(len(virtual_vertical_lines1[1:])):
        mix = round((virtual_vertical_lines1[i+1]+virtual_vertical_lines2[i])/2)
        virtual_vertical_lines.append(mix)
    virtual_vertical_lines.append(virtual_vertical_lines2[-1])
    
    virtual_horizontal_lines.append(virtual_horizontal_lines1[0])
    for i in range(len(virtual_horizontal_lines1[1:])):
        mix = round((virtual_horizontal_lines1[i+1]+virtual_horizontal_lines2[i])/2)
        virtual_horizontal_lines.append(mix)
    virtual_horizontal_lines.append(virtual_horizontal_lines2[-1])
...
def evaluate_image_generation(context1, gt_context1):
    ...
    # html format process
    htmlContext1 = htmlPostProcess1(context1)
    #htmlContext2 = htmlPostProcess(context2)
    htmlGtContext2 = htmlPostProcess2(gt_context1)
    #htmlGtContext2 = htmlPostProcess(gt_context2)
    # Acc Evaluate
    if htmlContext1== htmlGtContext2:
        flag = 1
    else:
        flag = 0
...
if __name__ == "__main__": 
    ...
    stru_gt_path = '/data1/lcpang/lc/project_table/dataset/Fintabnet/ftn_test_struhtml_gt_wohead.pkl'
    save_folder = f'/data1/lcpang/lc/project_table/master_project/result/query_inst/result/CellBox/FTN/Convert/'
    for ithr in range(1,10):
        thr = ithr/100
        queryinst_result = f'/data1/lcpang/lc/project_table/master_project/result/query_inst/result/CellBox/FTN/e20_nms{thr}_result_queryinst_ftntest10571.pkl'
       ...
       #### get structure result
        ...
        save_dict = dict()
        err_list = []
        save_file = save_folder+f'e20_nms{thr}_query_struhtml_result.pkl'
        ...
        list2filew(save_folder+f'e20_nms{thr}_err_list.txt',err_list)
        
        stru_pred_path = f'/data1/lcpang/lc/project_table/master_project/result/query_inst/result/CellBox/FTN/Convert/e20_nms{thr}_query_struhtml_result.pkl'
        ...
        list2filew(save_folder+f'e20{thr}acc.jsonl',save_list)    
        
        data=json_lines.reader(open(save_folder+f'e20{thr}acc.jsonl','r'))
        ...
        average_acc=total_acc/10571
        with open(save_folder+f'e20{thr}_average_acc.txt','w')as f:
